...
export const useMagicblockWebSocket = (
  selectedFeed?: PriceFeed,
): UseMagicblockWebSocketResult => {
  ...
  const parseAccountData = useCallback(
    (accountInfo: AccountInfo<Buffer> | null): number | null => {
    ...
    async (feedAddress: PublicKey) => {
      ...
      try {
        ...
        if (subscriptionIdRef.current !== null) {
          await connectionRef.current.removeAccountChangeListener(
        ...
        const accountInfo = await connectionRef.current.getAccountInfo(
          feedAddress,
        );
        ...
        setError(
          err instanceof Error ? err.message : "Failed to subscribe to account",
        );
  ...
  useEffect(() => {
    ...
    return () => {
      if (connectionRef.current && subscriptionIdRef.current !== null) {
        connectionRef.current.removeAccountChangeListener(
          subscriptionIdRef.current,
        );
      }
    };
  ...
  useEffect(() => {
    ...
    try {
      const feedAddress = deriveFeedAddress(
        selectedFeed.pyth_lazer_id.toString(),
      );
